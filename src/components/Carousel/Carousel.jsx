...
const Carousel = () => {
  ...
  const nextImageHandle = useCallback(() => {
    setShowItem((prevCount) => itemsNumber - prevCount === 0 ? 1 : prevCount + 1);
  }, [showItem]);

  const previousImageHandle = useCallback(() => {
    setShowItem((prevCount) => itemsNumber - prevCount === itemsNumber - 1 ? itemsNumber : prevCount - 1);
  }, [showItem]);
          ...
          list.map(item =>
            ...
            >
              <div className={scss.textBlock}>
                <ThirdTitle parentClass={scss.title}>
                  {item.title}
                </ThirdTitle>
